...
\significancestatement{Simulations are an important tool for investigating how brains work.
...
We demonstrate that using our new method we can not only simulate a very large brain model on a single GPU, but also do so up to \SI{35}{\percent} faster than in previous supercomputer simulations.}
...
In computer simulations of spiking neural networks, propagating spikes involves  adding the inputs of synapses from each spiking presynaptic neuron to the postsynaptic neurons.
...
The information which neurons are synaptically connected and with what weight is typically generated before a simulation is run and stored in large arrays. 
...
Fortunately, by careful design, these simulators can keep the memory requirements for each node constant, even when a simulation is distributed across thousands of nodes~\citep{Jordan2018}.
...
Unfortunately, propagating a spike in a spiking neural network simulation is likely to require accessing around \SI{8}{\byte} of memory but performing many fewer than the required 80 instructions. 
...
However, individual GPUs do not have enough memory to simulate truly large-scallarger brain models and, although small numbers of GPUs can be connected using the high-speed NVLink~\citep{NVIDIACorporation} interconnect, larger GPU clusters suffer from the same communication overheads as any other distributed HPC system.
...
We realised that, if each synaptic connection can be re-initialised in less than the 80 operations required to hide the latency incurred when fetching its \SI{8}{\byte} of parameter values from memory, it could be faster and vastly more memory efficient to regenerate synaptic connections on demand rather than storing them in memory.
This is the concept of procedural connectivity. It is applicable whenever synapses are static -- plastic synapses which change their weights during a simulation will have to be simulated in the traditional way.
...
However, for sparse connectivity, it is much more efficient to sample from the geometric distribution $\text{Geom}[P_{\text{conn}}]$ which governs the number of Bernoulli trials until the next success (i.e. a synapse).
...
However, if weights vary across synapses, the `bitfield' cannot be used either and the memory constraints for the 'sparse' representation become even more severe.
%
...
Furthermore, the simulation also is much slower for a large number of populations (Fig.~\ref{fig:merging_scaling}B).
%
\begin{figure}
    \centering
...
Furthermore, such an approach places the responsibility for optimising model structure for efficient simulation onto the modellers, who often prefer to concentrate on the science and organise populations according to anatomy or physiology.
...
\begin{figure*}
    ...
    \caption{Results of full-scale multi-area model simulation of resting state. 
    ...
    \textbf{E} Average pairwise correlation coefficients of spiking activity. 
...
Fig.~\ref{fig:multi_area}A-C shows some example spike rasters from the resting state simulation of three of the modelled areas, illustrating the characteristic irregular activity and population bursts.
