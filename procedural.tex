...
Small numbers of GPUs can be installed within a single node and connected using high-speed NVLink~\citep{NVIDIACorporation} interconnect, allowing them to communicate with low latency and high bandwidth~\citep{Li2020}.
However, larger GPU clusters use the same MPI based protocols and network fabric for inter-node communication as CPU based systems hence suffer from similar communication overheads.
...
Procedural connectivity uses the large amount of computational power available on a GPU to generate synaoptic connectivity `on the fly' -- reducing the memory and memory bandwidth requirements.
...
While procedural connectivity vastly reduces the memory requirements of large-scale brain simulations, previous GPU code generation strategies do not scale well to models containing large numbers of neural populations and synaptic projections.
To address this second problem, in section~\ref{sec:results/kernel_merging}, we describe our `kernel merging' code generator and demonstrate how it effectively addresses this issue.
