...
\end{lstlisting}
%
In CUDA, threads are grouped into `thread blocks', within which data can be exchanged via fast local memory.
However, to maximise performance, the code running on the threads within a thread block should not `diverge'.
To reduce this, we add `padding' threads around each population of neurons so, in the example, 100 neurons are padded to the next multiple of 32 (assumed to be the size of the thread block in this example).
This approach is similar to the ``Inter Thread Block fusion'' strategy described by \citet{Wang2010} and works well for a small number of populations.
However, as Fig.~\ref{fig:merging_scaling}A illustrates, when we partition a model consisting of \num{1000000} LIF neurons into an increasingly large number of ever smaller populations, compilation time increases super-linearly and quickly becomes impractical.
