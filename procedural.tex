...
The recent multi-area model~\citep{Schmidt2018a,Schmidt2018} does this -- using scaled versions of a previous 4 layer microcircuit model~\citep{Potjans2012} to implement \SI{1}{\milli\meter\squared} `patches' for each of 32 areas of the macaque cortex involved in visual processing.
...
Initialization our model took \SI{6}{\minute} -- \SI{3}{\minute} of which was spent generating and compiling code -- and each biological second of simulation took \SI{7.7}{\minute} -- 
Fig.~\ref{fig:multi_area}A-C shows some example spiking activity from three of the modelled areas.
Next, we compared the per-layer distributions of rates, spike-train irregularity and cross-correlation coefficients across all areas with those obtained from the supercomputer simulations.
Irregularity was calculated using the revised local variaation LvR~\citep{Shinomoto2009} averaged over a subsample of \num{2000} neurons and 
