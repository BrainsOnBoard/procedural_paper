...
\begin{itemize}
    \item lots of synapses!
    \item simulations of large-scale brain models require large amounts of memory and, due to this, are typically simulated on large distributed computer systems.
    \item neuromorphic hardware - solves this with lots of on-chip memory and reductions in precision / not supporting fan-in
    \item SpiNNaker solves this with lots of on-chip memory and SDRAM distributed across system
    \item SpiNNaker 2 aims to scale this up with faster more modern SDRAM - however ratio to compute/memory is marginally worse
    \item In future, more exotic memory technologies may prove to be the solution
    \item GPU in totally different space architecturaly - very poor compute/memory ratio - 10 instructions to 1 byte memory
    \item However, procedural approach
\end{itemize}
...
One particular relevant feature of the brain is that its memory is located throughout the system -- primarily in the synapses -- rather than being centrally located.
...
However, while SRAM is fast, it can only be fabricated at relatively low density meaning that many of these systems economize either by reducing the maximum number of synapses per neuron (256 ODIN) or by reducing the precision of the synaptic weights to \num{6}~\citep{Schemmel2017}, \num{4}~\citep{Frenkel2018} or even \SI{1}{\bit}~\citep{Merolla2014,Frenkel2019}.
Recent research has suggested that reduced weight precision may only have a minimal affect on the accuracy of spiking classifiers and that stochastic computation can be used to counteract the affects of limited precision in neural simulation~\citep{Hopkins2020} and learning~\citep{Frenkel2019,Seo2011}.
...
Similarly to the other neuromorphic systems, SpiNNaker~\citep{Furber2014} has a large amount of SRAM on each chip is a Rhodes2019
