...
However, in order to reproduce some types of activity found in recordings of biological brains, large models are required. 
...
In this work, we present a new method for simulating such models that massively reduces the amount of memory required and is particularly well-suited to use on Graphics Processing Units~(GPUs).
GPUs were originally developed for 3D graphics rendering, but have proven to be well-suited to accelerating many other tasks and are now a common fixture in many workstations.
Using a single GPU, we simulate a very large brain model \SI{35}{\percent} faster than a previous supercomputer simulation.}
...
\begin{table}
  ...
  \begin{tabular}{r S S S}
    ...
    \midrule
        $\tau_{m}$ [\si{\milli\second}]         & 20                        & 20                & 2 \\
        $V_{rest}$ [\si{\milli\volt}]           & -60.0                     & -70.0             & -65 \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Model parameters. \textsuperscript{1}~Excitatory/Inhibitory. \textsuperscript{2}~Please refer to original works\citep[Table 1,2]{Schmidt2018}}
...
In the procedural connectivity benchmark and the multi-area model where there are synaptic connections, incoming spikes induce an exponentially-shaped input current in $I_{{in}_{j}}$:
%
\begin{align}
...
In all models, the continuous terms of the Eq.~\ref{eq:lif_neuron}~and~\ref{eq:exp_neuron_input_current} are seperately solved algebraically so that the synaptic input current $I_{{in}_{j}}$ entering into equation~\ref{eq:lif_neuron} is treated as a constant during each simulation timestep.}
