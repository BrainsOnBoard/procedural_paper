...
We demonstrate that using our new method we can not only simulate a very large brain model on a single GPU, but also do so \SI{35}{\percent} faster than in previous supercomputer simulations.}
...
Our results match those obtained on a supercomputer and the simulation runs \SI{35}{\percent} faster on a single high-end GPU than previously on over 1000 supercomputer nodes.
\end{abstract}
...
This is the concept of procedural connectivity and is applicable whenever synapses are static -- plastic synapses which change their weights during a simulation will have to be simulated in the traditional way.
...
The profiler documentation suggests that these particular events are likely to be caused by ``Excessively jumping across large blocks of assembly code''~\citep[p47]{NVIDIACorporation2020}, which makes sense when we are generating kernels with hundreds of thousands of lines of code.
