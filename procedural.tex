...
Modern Graphical Processing Units~(GPUs) have relatively little on-chip memory and, instead, dedicate the majority of their silicon area to arithmetic logic units.
...
For example, the memory latency of a typical modern GPU can be completely hidden if each compute thread performs approximately 10 arithmetic operations per byte of data accessed from memory.
...
In this work, we present two methods which enable large-scale brain simulations on a single GPU workstation.
The first method, which we describe in section~\ref{sec:results/procedural}, we call `procedural connectivity'.
...
In previous works~\citep{Yavuz2016, Knight2018} we presented our GeNN simulator which uses code generation to convert neuron and synapse update algorithms -- described using `snippets' of C-like code -- into CUDA code for efficient GPU simulation.
...
In this paper, we are going one step further.
...
We call our second method `kernel merging' and it relates to the way these kernels are implemented.
...
%
\section{Discussion}
In this work we have presented two methods for large-scale brain simulation on GPU devices which entirely removes the need to store connectivity data in memory.
...
The raw data~\citep{Knight2020c} used to produce Fig.~\ref{fig:performance_scaling} and Fig.~\ref{fig:merging_scaling}; and the pre-processed data used to produce Fig.~\ref{fig:multi_area} are available at \url{https://doi.org/10.5281/zenodo.4277749}.
Furthermore, the raw data used to produce Fig.~\ref{fig:performance_scaling} and Fig.~\ref{fig:merging_scaling} are also available with this manuscript.
The raw spiking data from the GeNN simulations of the multi-area model~\citep{Knight2020b} are available at \url{https://doi.org/10.25377/sussex.12912699}.
\citet{Schmidt2018} hold the copyright for the raw spiking data from their
NEST simulations and the data is available from them upon request.

\section{Code availability}
All experiments were carried out using the GeNN 4.3.3~\citep{Knight2020}, available at \url{https://doi.org/10.5281/zenodo.4022384}.
A GeNN port of the multi-area model~\citep{VanAlbada2020b} is available at \url{https://doi.org/10.5281/zenodo.4271816}.
The models used to produce Fig.~\ref{fig:performance_scaling} and Fig.~\ref{fig:merging_scaling} as well as the code to produce all figures are available at \url{https://doi.org/10.5281/zenodo.4277749}.
The latest version of the code used for this paper is also availalble at \url{https://github.com/BrainsOnBoard/procedural_paper/}.
