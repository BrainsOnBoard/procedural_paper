...
fig, axes = plt.subplots(1, 4, figsize=(plot_settings.large_figure[0], plot_settings.small_figure[1]), frameon=False)

# Plot compile time
compile_time_axis = axes[0]
...
sol_time_axis.set_xlabel("Num populations")
sol_time_axis.set_ylabel("Memory utilization [%]")
...
stall_count_axis = axes[3]
stall_count_axis.set_title("D", loc="left")
stall_count_axis.plot(data[:,0], data[:,5])
stall_count_axis.plot(data[:,0], data[:,10])
stall_count_axis.set_xlabel("Num populations")
stall_count_axis.set_ylabel("Instruction stall count")
